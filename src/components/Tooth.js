...
import './Tooth.css'
import imagen from './fotodiente.png'
...
function Tooth({ number, positionX, positionY, onChange }) {
  ...
  const doneSubMenu = (place, value) => {
    return {
      ...
      'Resina total': () => dispatch(carie('all', value)),
      'Diente ausente': () => dispatch(extract(value)),
      Corona: () => dispatch(crown(value)),
      filtracion: () => dispatch(filter(value)),
  ...
  const todoSubMenu = (place, value) => {
    return {
      resina: () => dispatch(carie(place, value)),
      'Resina total': () => dispatch(carie('all', value)),
      'Diente ausente': () => dispatch(extract(value)),
      Corona: () => dispatch(crown(value)),
      filtracion: () => dispatch(filter(value)),
  ...
  const menuConfig = (place) => {
    return {
      realizado: doneSubMenu(place, 1),
      'Por realizar': todoSubMenu(place, 2),
      'JSX line': <hr></hr>,
      'Clear All': () => dispatch(clear()),
    }
  }

  const getClassNamesByZone = (zone) => {
